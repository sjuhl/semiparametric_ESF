...
# load matrices and standardize them (spectral normalization)
load("./W.RData")
...
grid <- matrix(as.matrix(cbind(expand.grid(p,W_id),seq_len(length(p)*length(W)))),ncol=3
               ,dimnames=list(NULL, c("p", "W_id","multi_id")))
...
input <- input[order(input$W_id,input$p,input$multi_id),]
...
sim_func(spmultiplier=multipliers[[input$multi_id[1]]]
         ,beta=b,sd.e=sd.e,W=W[[input$W_id[1]]],SEM=F,ideal.setsize=F)
...
# save output
save(input,sim_out,nsim,time.taken,session_info,file="./SimOut/MC_Output.RData")
